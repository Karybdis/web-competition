...
//    List<Competition> findtest(String year,String grade,String name,String student,String teacher);
    @Query(value = "select * from competition  where ((case when ?1='' then 1 end ) or year like %?1%) and ((case when ?2='' then 1 end ) or gradeLarge like %?2%) and ((case when ?3='' then 1 end ) or gradeSmall like %?3%) and ((case when ?4='' then 1 end ) or nameLarge like %?4%) and ((case when ?5='' then 1 end ) or nameSmall like %?5%) and ((case when ?6='' then 1 end ) or nameDetail like %?6%) and ((case when ?7='' then 1 end ) or student1 like %?7% or student2 like %?7% or student3 like %?7%) and ((case when ?8='' then 1 end ) or teacher1 like %?8% or teacher2 like %?8%) ",nativeQuery = true)
    List<Competition> findtest(String year,String gradeLarge,String gradeSmall,String nameLarge,String nameSmall,String nameDetail,String student,String teacher);
}